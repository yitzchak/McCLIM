      ...
      (let ((pv  (elt points (mod (- i 1) n)))  ;the point before
        ...
        (cond
          ...
          ((line-contains-point-p x1 y1 x2 y2 (point-x po) (point-y po))
           (let ((sign-1 (line-equation x1 y1 x2 y2 (point-x pn) (point-y pn)))
                 (sign-2 (line-equation x1 y1 x2 y2 (point-x pv) (point-y pv))))
             (cond ((or (and (> sign-1 0) (< sign-2 0))
                   ...
                   ((= sign-1 0)
                    ...
                    (let* ((px (point-x pnn))
                           (py (point-y pnn))
                           (sign-1 (line-equation x1 y1 x2 y2 px py)))
                      (cond ((or (and (> sign-1 0) (< sign-2 0))
                                 (and (< sign-1 0) (> sign-2 0)))
