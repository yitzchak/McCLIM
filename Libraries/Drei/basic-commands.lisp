...
(define-command com-insert-clipboard
    ()
  (when (typep *current-gesture* 'clime:selection-request-response-event)
    (let ((content (clime:selection-event-content *current-gesture*)))
...
(defmethod command-for-unbound-gestures ((view textual-drei-syntax-view) gestures)
  (cond ((and (= (length gestures) 1)
        ...
        ((typep (first gestures) 'clime:selection-request-response-event)
